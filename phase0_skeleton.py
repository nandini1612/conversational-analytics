...
FEATURE_SCHEMA = {
    ...
    "final_sentiment":      {"dtype": float, "min": -1.0, "max":  1.0},
    "sentiment_std":        {"dtype": float, "min":  0.0, "max":  2.0},
    # Group B — conversational structure
    "talk_time_ratio":      {"dtype": float, "min":  0.0, "max":  1.0},
    "avg_words_agent":      {"dtype": float, "min":  0.0, "max": None},
    "avg_words_customer":   {"dtype": float, "min":  0.0, "max": None},
    ...
    "issue_type_billing":       {"dtype": float, "min": 0.0, "max": 1.0},
    "issue_type_technical":     {"dtype": float, "min": 0.0, "max": 1.0},
    "issue_type_account":       {"dtype": float, "min": 0.0, "max": 1.0},
    "issue_type_payment":       {"dtype": float, "min": 0.0, "max": 1.0},
    "issue_type_network":       {"dtype": float, "min": 0.0, "max": 1.0},
    "issue_type_delivery":      {"dtype": float, "min": 0.0, "max": 1.0},
    "issue_type_returns":       {"dtype": float, "min": 0.0, "max": 1.0},
    "issue_type_complaint":     {"dtype": float, "min": 0.0, "max": 1.0},
    "issue_type_upgrade":       {"dtype": float, "min": 0.0, "max": 1.0},
    "issue_type_cancellation":  {"dtype": float, "min": 0.0, "max": 1.0},
}
...
def validate_feature_schema(df: pd.DataFrame, split_name: str = "unknown") -> bool:
    ...
    if extra:
        print(f"  [WARN] {split_name}: extra columns not in schema (will be ignored): {extra}")
    ...
    print(f"  [OK] {split_name}: {len(df)} rows × {len(present)} features — schema valid")
    return True
...
def dummy_features(n_rows: int, seed: int = RANDOM_STATE) -> tuple[np.ndarray, np.ndarray]:
    """
...
def run_skeleton():
    ...
    if rf_val_preds.std() < y_val.std() * 0.5:
        print("  [WARN] RF is compressing predictions toward the mean — expected behaviour.")
...
def demo_schema_validator():
    ...
    bad_data = {col: rng.random(n) for col in FEATURE_COLUMNS if col != "empathy_density"}
